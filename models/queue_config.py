...
class QueueConfiguration(models.Model):
    ...
    name = fields.Char('Nom de Configuration', required=True, default='Configuration Principale')
    
    # Paramètres de performance
    dashboard_refresh_interval = fields.Integer('Intervalle Actualisation Dashboard (sec)', default=15)
    ...
    allow_ticket_cancellation = fields.Boolean('Permettre Annulation Tickets', default=True)
    ...
    def apply_configuration(self):
        ...
        params_mapping = {
            'queue_management.default_refresh_interval': str(self.dashboard_refresh_interval),
            ...
            'queue_management.enable_notifications': str(self.enable_email_notifications),
...
class QueuePerformanceMetrics(models.Model):
    ...
    def collect_daily_metrics(self, target_date=None):
        ...
        for service in services:
            ...
            self.create({
                ...
                'capacity_utilization': self._calculate_capacity_utilization(service, stats),
                'peak_hour': self._get_peak_hour_from_stats(stats),
                'peak_load': max(stats.get('hourly_distribution', {}).values()) if stats.get('hourly_distribution') else 0
            })
        
        _logger.info(f"Métriques collectées pour {len(services)} services le {target_date}")
    ...
    def get_trends_analysis(self, days_back=30):
        ...
        for service in services:
            ...
            first_week = service_metrics[:7] if len(service_metrics) >= 7 else service_metrics
            last_week = service_metrics[-7:] if len(service_metrics) >= 7 else service_metrics
            
            first_avg_wait = sum(m.avg_waiting_time for m in first_week) / len(first_week)
            last_avg_wait = sum(m.avg_waiting_time for m in last_week) / len(last_week)
            
            first_completion = sum(m.completion_rate for m in first_week) / len(first_week)
            last_completion = sum(m.completion_rate for m in last_week) / len(last_week)
            
            trends[service.id] = {
                'service_name': service.name,
                'wait_time_trend': self._calculate_trend_direction(first_avg_wait, last_avg_wait),
                'completion_trend': self._calculate_trend_direction(first_completion, last_completion),
                'total_tickets_period': sum(m.total_tickets for m in service_metrics),
                'avg_satisfaction': sum(m.satisfaction_rate for m in service_metrics if m.satisfaction_rate > 0) / 
                                 len([m for m in service_metrics if m.satisfaction_rate > 0]) if service_metrics else 0
            }
...
class QueueOptimizations(models.Model):
    ...
    def create_database_indexes(self):
        ...
        indexes_to_create = [
            ...
            ('queue_ticket_number_service_idx', 'queue_ticket', ['ticket_number', 'service_id']),
    ...
    def analyze_query_performance(self):
        ...
        # Requêtes à analyser
        test_queries = [
            ("Dashboard data", "SELECT COUNT(*) FROM queue_ticket WHERE state = 'waiting'"),
            ("Service stats", "SELECT service_id, COUNT(*) FROM queue_ticket WHERE created_time >= CURRENT_DATE GROUP BY service_id"),
            ("Waiting tickets", "SELECT * FROM queue_ticket WHERE state = 'waiting' ORDER BY ticket_number LIMIT 10")
        ]
        ...
        for query_name, query in test_queries:
            try:
                ...
                performance_results.append({
                    ...
                    'status': 'fast' if execution_time < 100 else 'slow' if execution_time < 500 else 'very_slow'
                })
    ...
    def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            if execution_time > 1000:  # Plus d'1 seconde
                _logger.warning(f"Méthode lente détectée: {func.__name__} - {execution_time:.2f}ms")
            
            return result
            
        except Exception as e:
            execution_time = (time.time() - start_time) * 1000
            _logger.error(f"Erreur dans {func.__name__} après {execution_time:.2f}ms: {e}")
            raise
