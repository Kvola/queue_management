...
class QueueController(http.Controller):
    ...
    @http.route('/queue/print_ticket/<int:ticket_id>', type='http', auth='public', website=True)
    def print_ticket(self, ticket_id):
        """Version imprimable du ticket"""
        ticket = request.env['queue.ticket'].sudo().browse(ticket_id)
        if not ticket.exists():
            return request.not_found()
    ...
    def cancel_ticket(self, ticket_number, service_id, reason='', **kwargs):
        ...
        try:
            ...
            if not ticket_number or not service_id:
                return {'error': 'Numéro de ticket et ID de service requis'}
            ...
            if not ticket:
                return {'error': 'Ticket non trouvé ou déjà traité'}
            ...
            if config and not config.allow_ticket_cancellation:
                return {'error': 'Annulation non autorisée par la configuration'}
            
            # Annuler le ticket
            cancel_reason = f"Annulé par le client. Raison: {reason}" if reason else "Annulé par le client"
            ticket.write({
                'state': 'cancelled',
                'notes': cancel_reason
            })
            
            return {'success': True, 'message': 'Ticket annulé avec succès'}
